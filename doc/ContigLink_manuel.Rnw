...
\subsection{Déroulement d'une analyse: Exemple pour les données de séquences mitochondrial de l'orge (projet C. Rizzolati)}

\subsubsection{Instancier la classe et estimer les tailles d'inserts}

<<CLV1, echo=TRUE, eval=FALSE>>=

# Instancier un objet de la class ContigLinkViewer

CLV <- new("ContigLinkViewer", 
           BamFile = "Fm2pc_MP_mate_sorted_F8_F4.bam", 
...
\subsubsection{Visualizer les distribution des tailles d'inserts}
...
ggplot(data=CLV@dfSame) +
  geom_histogram(aes(x=Isize),binwidth=1000) + 
